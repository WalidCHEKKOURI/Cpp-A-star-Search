...
#include <algorithm>
/*
A demo to showcase the A* Search Algorithm
*/
...
bool CheckValidCell(int x, int y, vector<vector<State>>& grid) 
{
	...
	return false;//coordinate not available on the grid
}
...
{ 
	...
	while (open.size() > 0)//while open vector is non empty
	{
		...
		open.pop_back();
		x = current[0];//Get the x 
		y = current[1];//and y values from the current node,
